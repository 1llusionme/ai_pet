...
import { X, Sticker, Image as ImageIcon, Rocket, Star } from "lucide-react";
import { useState, useRef } from "react";

interface PasteModalProps {
  isOpen: boolean;
  onClose: () => void;
  onLearn: (content: string, isImage?: boolean) => void;
}

export function PasteModal({ isOpen, onClose, onLearn }: PasteModalProps) {
  ...
  const handleLearn = () => {
    if (view === "paste" && content.trim()) {
      ...
      closeModal();
    } else if (view === "image" && previewImage) {
      onLearn("图片已上传，请提问", true);
      closeModal();
    }
  };
    ...
    <AnimatePresence>
      {isOpen && (
        ...
        >
          ...
          >
            {/* Header - Now simplified with just a close button */}
