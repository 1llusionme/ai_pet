...
function normalizeNudgeQuestion(message: string) {
  const compact = String(message || "").replace(/\s+/g, " ").trim();
  if (!compact) {
    return "";
  }
  const withoutPrefix = compact.replace(/^【[^】]+】\s*/g, "").trim();
  if (!withoutPrefix) {
    return "";
  }
  return withoutPrefix.endsWith("？") || withoutPrefix.endsWith("?") ? withoutPrefix : `${withoutPrefix}？`;
}

function fingerprintNudge(message: string) {
...
export default function ChatPage() {
  ...
  const [nudgeDraftText, setNudgeDraftText] = useState("");
  const [nudgeDraftVersion, setNudgeDraftVersion] = useState(0);
  ...
  const recentNudgeFingerprintsRef = useRef<string[]>([]);
  ...
  useEffect(() => {
    ...
    const rememberNudge = (content: string) => {
      const fingerprint = fingerprintNudge(content);
      if (!fingerprint) {
        return false;
      }
      const current = recentNudgeFingerprintsRef.current;
      if (current.includes(fingerprint)) {
        return false;
      }
      recentNudgeFingerprintsRef.current = [...current, fingerprint].slice(-8);
      return true;
    };
    const pollNotification = async () => {
      try {
        ...
        const incoming = fingerprintNudge(notification.content);
        const currentOpen = fingerprintNudge(nudgeMessage || "");
        if (!incoming || incoming === currentOpen) {
          return;
        }
        if (!rememberNudge(notification.content)) {
          return;
        }
    ...
    return () => window.clearInterval(timer);
  }, [conversationId, nudgeMessage, userId]);
  ...
  const handleExportLearningCard = useCallback(
    async (message: Message) => {
      ...
      try {
        ...
        const primaryTypeLabel = exportInfo.primary_card_type ? typeLabelMap[exportInfo.primary_card_type] || "学习卡" : "学习卡";
        const summaryOnly = exportInfo.experience?.status === "summary_only";
        const llmCards = exportInfo.generation?.llm_cards || 0;
        const generationHint =
          llmCards > 0 ? `，其中${llmCards}张为智能润色` : exportInfo.generation?.llm_enabled ? "，本次使用规则护栏生成" : "";
        const feedback =
          exportInfo.experience?.message ||
          (summaryOnly
            ? "已生成学习主卡，本次未生成重点卡。"
            : `已转学习卡片：主卡类型${primaryTypeLabel}，新增${exportInfo.term_cards_added}，更新${exportInfo.term_cards_updated}，错题联动${exportInfo.review_records_created}${generationHint}`);
        const guidance = exportInfo.experience?.next_action || "你可以在学习资产库继续查看并复习。";
  ...
  const handleNudgeClick = () => {
    if (nudgeMessage) {
      const draft = normalizeNudgeQuestion(nudgeMessage);
      if (draft) {
        setNudgeDraftText(draft);
        setNudgeDraftVersion((prev) => prev + 1);
      }
    ...
    <div className="flex flex-col h-screen w-full bg-[#FEF2F2] sm:p-6 md:p-10 overflow-hidden items-center justify-center font-sans">
      ...
      <div className="relative w-full h-full sm:max-w-[400px] sm:max-h-[850px] bg-[#FAFAF9] sm:border-[12px] sm:border-white overflow-hidden flex flex-col sm:rounded-[56px] z-10 isolate shadow-[0_20px_50px_rgba(0,0,0,0.05),_0_0_0_1px_rgba(0,0,0,0.02)]">
        ...
        <InputArea
          ...
          draftText={nudgeDraftText}
          draftVersion={nudgeDraftVersion}
        />
        <PasteModal
